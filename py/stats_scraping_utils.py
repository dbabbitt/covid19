...
class StatsScrapingUtilities(object):
    ...
    def __init__(self, s=None, verbose=False):
        ...
        # df.disease_name = df.disease_name.map(lambda x: ssu.disease_name_dict.get(x, x))
    ...
    def check_for_typos(self, left_list, right_list, verbose=False):
        '''
        ...
        for left_item in left_list:
            ...
            for right_item in right_list:
                this_similarity = self.get_similarity_measure(left_item, right_item)
                if this_similarity > max_similarity:
                    max_similarity = this_similarity
                    max_item = right_item
    ...
    def conjunctify_nouns(self, noun_list=None, verbose=False):
        if not noun_list:
            list_str = ''
